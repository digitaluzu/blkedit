...
public class PlayerPrefsEditor : EditorWindow
{
	...
	/// <summary> Whether to display the section for adding a new entry </summary>
	bool								m_ShowNewEntry = false;
	
	/// <summary> Values for new entry fields </summary>
	string								m_NewEntry_Name = "";
	string								m_NewEntry_Value = "";
	EntryType							m_NewEntry_Type = EntryType.Int;
	
	/// <summary> Whether to display search filter section </summary>
	...
	void OnGUI() 
	{
		...
		if (m_Plist != null)
		{
			...
			EditorGUILayout.BeginHorizontal();
				...
				if (Button("Refresh", 80.0f, Color.gray))
				{
					Load();
				}
				EditorGUILayout.LabelField("Edit in Play Mode", GUILayout.Width(100));
				m_CanEditInPlayMode = EditorGUILayout.Toggle(m_CanEditInPlayMode, GUILayout.Width(20));
			GUILayout.Space(10.0f);
			EditorGUILayout.EndHorizontal();
			
			// New Entry			
			m_ShowNewEntry = EditorGUILayout.Foldout(m_ShowNewEntry, "Add");
			if (m_ShowNewEntry)
			{
				EditorGUILayout.BeginVertical();
					m_NewEntry_Type = (EntryType) EditorGUILayout.EnumPopup("Type:", m_NewEntry_Type, GUILayout.Width(widthValue));
					m_NewEntry_Name = EditorGUILayout.TextField("Key:", m_NewEntry_Name, GUILayout.Width(widthValue));
					m_NewEntry_Value = EditorGUILayout.TextField("Value:", m_NewEntry_Value, GUILayout.Width(widthValue));

					EditorGUILayout.BeginHorizontal();
	
						if (Button("Cancel", 80.0f, Color.red))
						{
							m_ShowNewEntry = false;
						}
						if (Button("Add", 80.0f, Color.green))
						{
							string errorMsg;
							if (AddNewEntry(out errorMsg))
							{
								Save();
								Load();
								m_NewEntry_Name = "";
							...
							{
								EditorUtility.DisplayDialog("Could not add entry", errorMsg, "OK");								
							}
						}
					EditorGUILayout.EndHorizontal();
			...
			// Search filter
			m_ShowFilter = EditorGUILayout.Foldout(m_ShowFilter, "Search");
			if (m_ShowFilter)
			{
				EditorGUILayout.BeginHorizontal();
	...
	bool OnGUIEntryValue(KeyValuePair<string, object> entry, float width)
	{
		...
		{
			float newValue = EditorGUILayout.FloatField(PlayerPrefs.GetFloat(entry.Key), GUILayout.MaxWidth(width));
			if (newValue != (float)((double)entry.Value))
			{
				m_Plist[entry.Key] = newValue;
				PlayerPrefs.SetFloat(entry.Key, newValue);
				return true;
	...
	/// <summary> Adds a new entry into the player prefs </summary>
	bool AddNewEntry(out string errorMsg)
	{
		errorMsg = "";
		
		// Make sure an entry with the same name doesn't exist already
		if (PlayerPrefs.HasKey(m_NewEntry_Name))
		{
			errorMsg = "An entry with the same name (" + m_NewEntry_Name + ") already exists!";
			return false;
		}
		
		// Make sure key isn't empty
		if (string.IsNullOrEmpty(m_NewEntry_Name))
		{
			errorMsg = "Please specify a valid key";
			return false;			
		}	
		
		switch (m_NewEntry_Type)
		{
			case EntryType.Int:
			{
				int newValue;
				if (System.Int32.TryParse(m_NewEntry_Value, out newValue))
				{
					PlayerPrefs.SetInt(m_NewEntry_Name, newValue);
					return true;
				}
				else
				{
					errorMsg = "Value (" + m_NewEntry_Value + ") could not be converted to specified type (" + m_NewEntry_Type.ToString() + ")";
				}
			} break;
			...
			{
				float newValue;
				if (System.Single.TryParse(m_NewEntry_Value, out newValue))
				{
					PlayerPrefs.SetFloat(m_NewEntry_Name, newValue);
					return true;
				}
				else
				{
			...
			case EntryType.Text:
			{
				PlayerPrefs.SetString(m_NewEntry_Name, m_NewEntry_Value);
				return true;
			}			
		}
